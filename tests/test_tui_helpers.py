...
from avanza_mcp.records import flattened_search_hits, parse_transaction_types
...
def test_portfolio_profit_summary_treats_negative_acquired_value_as_cost_basis():
    positions = {
        "withOrderbook": [
            {
                "account": {"id": "acc-1"},
...
def test_account_metric_values_day_falls_back_to_portfolio_day_when_account_day_missing():
    account = {
        "id": "acc-1",
        "totalValue": {"value": 5000, "unit": "SEK"},
        "buyingPower": {"value": 1000, "unit": "SEK"},
    ...
    metrics = account_metric_values(account, positions, "acc-1", "day")
