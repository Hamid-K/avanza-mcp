...
Stop-loss previews now show trigger validity, triggered-order valid days, and derived triggered-order expiry (`if triggered today`).
...
If `ERROR` shows `Ogiltigt giltighetsdatum`, the generated triggered-order validity was invalid. Replace with `order_valid_days=1`.
