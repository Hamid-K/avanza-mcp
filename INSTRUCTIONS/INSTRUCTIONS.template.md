...
- Before any optional BUY, name entry, thesis invalidation, gap allowance, exact `Antal`, and risk-at-invalidation. Cap new account risk at `0.75%` for quality/core growth, `0.50%` for high-beta/event-gated growth, and `0.25%` for speculative exposure; aggregate active optional BUY risk must remain at or below `2%`. If recent volatility is at least `1.5x` its six-month norm or a binary event makes loss estimates unreliable, cut the applicable cap by at least half rather than tightening a core stop inside normal noise.
