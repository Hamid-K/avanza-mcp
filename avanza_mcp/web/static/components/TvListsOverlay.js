  ...
  setup(props, { emit }) {
    ...
    async function load() {
      ...
      try {
        ...
        if (!selected.value && lists.value.length) selected.value = String(lists.value[0].id || lists.value[0].list_id || "");
      } catch (exc) {
    ...
    return { props, emit, lists, selected, rows, loading, error, columns, load };
  },
  template: `
    <div v-if="props.open" class="overlay fade-in" role="dialog" aria-modal="true">
      ...
      <div class="overlay-body">
        ...
        <DataTable v-else :columns="columns" :rows="rows" emptyText="No symbols (TradingView session may be missing)" />
      </div>
    </div>
  `,
});
