...
def validate(payload: dict[str, Any]) -> list[str]:
    ...
    if len(ids) != len(set(ids)):
        errors.append("scheduler active row IDs must be unique")
    for row in rows:
    ...
    if validation.get("canonical_approval_c_rows") != 18:
    ...
    terminal_count = int(validation.get("terminal_rows_in_active_section", 0) or 0)
